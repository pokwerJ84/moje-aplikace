(() => {
  ...
  const languageLabel = (lang = languageSetting) => lang === "cs-CZ" ? "Čeština" : lang === "en-US" ? "English" : "Auto";
  ...
  function detectLanguage(text) {
    ...
    const czechWords = new Set(["a","aby","ale","ani","ano","asi","bez","by","byl","byla","byli","co","do","ho","i","jak","je","jeho","její","jen","jsem","jsi","jsme","jsou","k","když","který","která","které","má","mám","mezi","mi","může","na","nad","ne","nebo","není","o","od","po","pod","pro","před","se","si","s","ta","tak","také","ten","to","u","už","v","ve","z","za","že"]);
    let score = 0;
    for (const w of words) if (czechWords.has(w)) score += 1;
  ...
  function setIdleStatus(actualLanguage = null) {
    const lang = actualLanguage ? languageLabel(actualLanguage) : languageLabel();
  ...
  synth.speak = function (utterance) {
    ...
    const actualLanguage = detectLanguage(text);
    unlockAudio();
    const myId = ++requestId;
    cancelled = false;
    activeUtterance = utterance;
    resetCloudAudio();
    setStatus(`${modeLabel()} · loading · ${languageLabel(actualLanguage)}`);
    ...
    (async () => {
      try {
        const response = await fetch(ENDPOINT, {
          method: "POST",
          headers: { "Content-Type": "application/json", "apikey": API_KEY },
          body: JSON.stringify({ text, mode, voice: cloudVoice, language: actualLanguage })
        });
  ...
  window.__pokyCloudTTS = { refreshUsage, version: "v13", get mode() { return mode; }, get language() { return languageSetting; } };
})();
