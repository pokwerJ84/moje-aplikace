(() => {
  ...
  function setIdleStatus() { setStatus(`${modeLabel()} · ${cloudVoiceLabel()}`); }
  function cleanupUrl() { if (objectUrl) { URL.revokeObjectURL(objectUrl); objectUrl = ""; } }
  function resetAudio() {
    try { audio.pause(); } catch (_) {}
    audio.onended = null; audio.onerror = null; audio.onplaying = null; cleanupUrl();
  }
  ...
  async function responseError(response) {
    try { const data = await response.json(); return data?.detail || data?.error || `HTTP ${response.status}`; }
  ...
  function populateVoiceSelect() {
    ...
    options.forEach(([value, label]) => {
      ...
      o.value = `cloud:${value}`; o.textContent = label; select.appendChild(o);
    });
  ...
  function syncModeUI() {
    ...
    if (modeSelect) {
      ...
      const freeOption = modeSelect.querySelector('option[value="free"]');
      if (freeOption) freeOption.textContent = "Free Cloud";
    }
    const version = document.getElementById("appVersion");
    if (version) version.textContent = "Version v11";
    populateVoiceSelect(); setIdleStatus(); refreshUsage();
  }

  synth.getVoices = function () {
    return [{ name: `${modeLabel()} ${cloudVoiceLabel()}`, lang: "en-US", voiceURI: `poky:${mode}:${cloudVoice}`, default: true }];
  };
  synth.cancel = function () {
    requestId += 1; cancelled = true; resetAudio(); activeUtterance = null; setIdleStatus();
  };
  ...
  synth.speak = function (utterance) {
    ...
    cancelled = false; activeUtterance = utterance; resetAudio(); setStatus(`${modeLabel()} · loading…`);
    (async () => {
      try {
        const response = await fetch(ENDPOINT, {
          method: "POST",
        ...
        const limit = Number(response.headers.get("x-tts-limit") || 900000);
        renderUsage(used, remaining, limit);
        const blob = await response.blob();
        if (!blob.size) throw new Error("Poky Voice returned empty audio.");
        cleanupUrl(); objectUrl = URL.createObjectURL(blob); audio.src = objectUrl;
        audio.playbackRate = Math.max(0.75, Math.min(1.5, Number(utterance.rate || 1))); audio.muted = false;
        let started = false;
        audio.onplaying = () => {
          if (started || myId !== requestId || cancelled) return;
          started = true; setIdleStatus();
          try { utterance.onstart?.({ type: "start" }); } catch (_) {}
        };
        audio.onended = () => {
          if (myId !== requestId || cancelled) return;
          activeUtterance = null; cleanupUrl(); setIdleStatus(); refreshUsage();
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    modeSelect?.addEventListener("change", () => {
      synth.cancel(); mode = modeSelect.value; localStorage.setItem(MODE_KEY, mode); syncModeUI();
    });
    voiceSelect?.addEventListener("change", () => {
      if (voiceSelect.value.startsWith("cloud:")) {
        cloudVoice = voiceSelect.value.slice(6); localStorage.setItem(CLOUD_VOICE_KEY, cloudVoice);
      }
      setIdleStatus();
    });
    syncModeUI();
  ...
  window.__pokyCloudTTS = { refreshUsage, version: "v11", get mode() { return mode; } };
})();